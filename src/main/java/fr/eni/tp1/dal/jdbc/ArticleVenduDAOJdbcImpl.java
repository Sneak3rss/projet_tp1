...
public class ArticleVenduDAOJdbcImpl implements DAOArticleVendu{
    ...
    private final static String SELECT_BY_ID = """
    		...
    		SELECT ARTICLES_VENDUS.no_article , ARTICLES_VENDUS.nom_article ,ARTICLES_VENDUS.description, ARTICLES_VENDUS.date_debut_encheres , ARTICLES_VENDUS.date_fin_encheres, ARTICLES_VENDUS.prix_initial , ARTICLES_VENDUS.prix_vente, ARTICLES_VENDUS.etat_vente, UTILISATEURS.no_utilisateur,ARTICLES_VENDUS.no_categorie,CATEGORIES.libelle,UTILISATEURS.pseudo,nom,RETRAITS.rue,RETRAITS.code_postal,RETRAITS.ville
            FROM ARTICLES_VENDUS 
            INNER JOIN CATEGORIES ON ARTICLES_VENDUS.no_categorie = CATEGORIES.no_categorie
		    INNER JOIN UTILISATEURS ON ARTICLES_VENDUS.no_utilisateur = UTILISATEURS.no_utilisateur
    ...
    public List<ArticleVendu> selectAll() {
    	...
    	try (Connection cnx = ConnectionProvider.getConnection()) {
    		...
    		while (rstSet.next()) {
    			...
    			ArticleVendu articleVendu= new ArticleVendu(no_article,nom_article, description,date_debut_encheres.toLocalDate(), date_fin_encheres.toLocalDate(), prix_initial, prix_vente, etat_vente);
		...
		ArticleVendu articleVendu= null;
	    	try (Connection cnx = ConnectionProvider.getConnection()) {
	    		...
	    		while (rstSet.next()) {
	    			...
	    			articleVendu= new ArticleVendu(nom_article, description,date_debut_encheres.toLocalDate(), date_fin_encheres.toLocalDate(), prix_initial, prix_vente, etat_vente);
	    			articleVendu.setNoarticle(noarticle);
	    			articleVendu.setRetrait(new Retrait(noarticle, rue, codePostal, ville));
